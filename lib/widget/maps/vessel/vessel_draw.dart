...
class _VesselDrawerState extends State<VesselDrawer> {
  ...
  Future<void> loadVesselParameters() async {
    try {
      ...
      length = double.parse(vesselShape.getElement('LengthM')!.getAttribute('Value').toString());
      width = double.parse(vesselShape.getElement('WidthM')!.getAttribute('Value').toString());

      final bowLine = document.findAllElements('BowLine').first;
      bowLineColor = Color(int.parse(bowLine.getAttribute('Color').toString()));

      lengthBowToBreakM = double.parse(vesselShape.getElement('LengthBowToBreakM')!.getAttribute('Value').toString());
    } catch (e) {
            ...
            child: SingleChildScrollView(
              ...
              child: Container(
                ...
                child: CustomPaint(
                  ...
                  painter: VesselPainter(length, width, lengthBowToBreakM, bowLineColor),
                ),
              ),
            ),
          );
...
class VesselPainter extends CustomPainter {
  ...
  VesselPainter(this.length, this.width, this.lengthBowToBreakM, this.bowLineColor);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double hullHeight = double.parse((3 * scale).toString()); // Adjust this ratio as needed

    // Draw the boat's hull (rectangle)
    path.addRect(Rect.fromPoints(const Offset(0, 0), Offset(lengthBow * scale, hullHeight)));
    ...
    canvas.drawLine(Offset(0, hullHeight + margin), Offset(lengthBow * scale, hullHeight + margin), paint);
    canvas.drawLine(Offset(0, hullHeight + margin - 5), Offset(0, hullHeight + margin + 5), paint);
    canvas.drawLine(
        Offset(lengthBow * scale, hullHeight + margin - 5), Offset(lengthBow * scale, hullHeight + margin + 5), paint);
    TextPainter(
      text: TextSpan(
      ...
      ..paint(canvas, Offset((lengthBow * scale) / 2 - 20, hullHeight + margin + 5));

    final marginBelowLine = margin * 2.5;

    canvas.drawLine(Offset(0, hullHeight + marginBelowLine), Offset(hullWidth, hullHeight + marginBelowLine), paint);
    canvas.drawLine(Offset(0, hullHeight + marginBelowLine - 5), Offset(0, hullHeight + marginBelowLine + 5), paint);
    ...
    TextPainter(
      ...
      ..layout(maxWidth: size.width)
      ..paint(canvas, Offset(hullWidth / 2 - 20, hullHeight + marginBelowLine + 5));

    // Draw vertical line for vessel width
    canvas.drawLine(const Offset(0 - 15, 0), Offset(0 - 15, hullHeight), paint);
    canvas.drawLine(const Offset(0 - 15 + 5, 0), const Offset(0 - 15 - 5, 0), paint);
    canvas.drawLine(Offset(0 - 15 + 5, hullHeight), Offset(0 - 15 - 5, hullHeight), paint);
    ...
    for (double i = -5; i <= numWidthLines + 5; i++) {
      ...
      canvas.drawLine(Offset(-((hullWidth * 1.3) - hullWidth), x), Offset(hullWidth * 1.3, x), paint);
    }

    // Draw horizontal grid lines for length
    for (double i = -9; i <= numLengthLines - 6; i++) {
      final y = i * gridSpacing;
      canvas.drawLine(Offset(y, -((hullHeight * 1.55) - hullHeight)), Offset(y, hullHeight * 2), paint);
    }
  }
